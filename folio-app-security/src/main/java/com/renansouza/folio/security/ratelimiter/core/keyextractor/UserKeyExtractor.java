...
/**
 * Extracts user-based keys from authenticated HTTP requests for identification or rate limiting purposes.
 * <p>
 * This implementation relies on the servlet container's authentication mechanism to obtain
 * user identity through {@link HttpServletRequest#getUserPrincipal()}. The extracted key
 * is prefixed with "user:" for consistent key formatting.
 * </p>
 *
...
public class UserKeyExtractor implements KeyExtractor {
     ...
     * <p>
     * This method retrieves the authenticated user's identity from the request's
     * {@link Principal} object and formats it as a prefixed key string.
     * </p>
     *
     ...
     * @throws IllegalArgumentException if no authenticated user is found in the request
     *         (i.e., when {@link HttpServletRequest#getUserPrincipal()} returns {@code null})
