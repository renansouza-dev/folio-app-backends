...
/**
 * A Spring MVC {@link HandlerInterceptor} implementation that provides HTTP request rate limiting
 * functionality.
 * <p>
 * This interceptor intercepts incoming HTTP requests before they reach the controller handlers and
 * applies rate limiting logic to prevent abuse and ensure fair resource usage. The interceptor
 * operates at the Spring MVC level, making it effective for all HTTP requests processed by the
 * DispatcherServlet.
 * </p>
 * <p>
 * The rate limiting is typically applied based on client identification criteria such as IP
 * address, user authentication, or custom headers. When a client exceeds the configured rate limit,
 * the interceptor blocks the request and returns an appropriate HTTP error response.
 * </p>
 * <p>
 * <strong>Usage:</strong><br>
 * This interceptor is automatically configured when the rate limiter library is included in a
 * Spring Boot application. It integrates seamlessly with Spring MVC's interceptor chain and
 * requires no manual configuration in most cases.
 * </p>
 * <p>
...
public class RateLimiterInterceptor implements HandlerInterceptor {
   ...
   * @param response the current HTTP response - used to set error status if rate limit exceeded -
   *                 must not be {@code null}
   * @param handler  the chosen handler to execute, for type and/or instance examination - must not
   *                 be {@code null}
  ...
  public boolean preHandle(@NonNull HttpServletRequest request,
    ...
    try {
      if (!(handler instanceof HandlerMethod method) || !rateLimiterProperties.isEnabled()) {
        return true;
      }
