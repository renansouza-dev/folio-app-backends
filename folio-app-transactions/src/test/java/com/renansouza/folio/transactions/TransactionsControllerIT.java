...
class TransactionsControllerIT {
  ...
  void getTransactionsByBroker() {
    ...
    var filteredTransactions = transactions.stream()
        .filter(t -> t.getBroker().equals(transactions.getLast().getBroker())).toList();
  ...
  void getTransactionsByAsset() {
    ...
    var filteredTransactions = transactions.stream()
        .filter(t -> t.getAsset().equals(transactions.getLast().getAsset())).toList();
  ...
  void shouldNotDeleteATransactions() {
    ...
    given()
        ...
        .body("", Matchers.aMapWithSize(5),
            ...
            "message",
            Matchers.equalTo(String.format("The provided id %s transaction was not found", id)),
