...
class TransactionsControllerTest {
  ...
  void getZeroTransactionsByBroker() throws Exception {
    ...
    var byBrokers = getResponses(10).stream()
        .filter(transaction -> broker.equals(transaction.broker())).toList();
    when(service.find(eq(broker), any(), any(PageRequest.class))).thenReturn(
        new PageImpl<>(byBrokers));

    // Then
    mvc.perform(
            get(PATH).param("broker", String.valueOf(broker)).accept(MediaType.APPLICATION_JSON))
  ...
  void getTransactions() throws Exception {
    // Given
    when(service.find(any(), any(), any(PageRequest.class))).thenReturn(
        new PageImpl<>(getResponses(3)));
  ...
  void getTransactionsByBroker() throws Exception {
    ...
    var byBrokers = responseList.stream().filter(transaction -> broker.equals(transaction.broker()))
        .toList();
  ...
  void getTransactionsByAsset() throws Exception {
    ...
    var byAssets = getResponses(5).stream().filter(transaction -> asset.equals(transaction.asset()))
        .toList();

    when(service.find(any(), eq(asset), any(PageRequest.class))).thenReturn(
        new PageImpl<>(byAssets));
  ...
  void addTransaction() throws Exception {
    ...
    // Then
    mvc.perform(post(PATH).content(mapper.writeValueAsString(getRequests(1).getFirst()))
